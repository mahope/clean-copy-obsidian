...
set -e
ZIP="$(dirname "$0")/../site/downloads/eaa-scanner-desktop-src-1.1.1.zip"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

echo "== 1. unzip =="
unzip -q "$ZIP" -d "$TMP"
test -f "$TMP/desktop/package.json" || { echo "FEJL: package.json mangler i zippens rod/desktop"; exit 1; }
echo "OK"
